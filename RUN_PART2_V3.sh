...
#
#  Arm semantics (same DCIGateV3, same feature stream, three routing arms):
#      DCI_FORCE=full   → always-FULL  (5-D + S_P every window — reference)
#      (unset)          → GATED        (the paper's method, rho=0.35)
#      DCI_FORCE=cheap  → always-CHEAP (union-Bonferroni only; S_P never)
#  The router's diagnostics stay logged in every arm (route pinned only).
#
...
for i in 0 1 2; do
  ...
  if [[ " $ENGINES " == *" pg "* ]]; then
    ...
    if [ -e "$DEST" ] && [ "${PART2_FORCE:-0}" != "1" ]; then
      ...
      DCI_GATE=v3 DCI_FORCE="$FORCE" PROBE_TAG="_p2v3_${TAG}" \
  ...
  if [[ " $ENGINES " == *" mg "* ]]; then
    ...
    if compgen -G "mongo/out/${SUB}/*" > /dev/null && [ "${PART2_FORCE:-0}" != "1" ]; then
      ...
      echo "── MG  DCI_GATE=v3 force=[$FORCE] → mongo/out/${SUB}/  ($(date -u +%H:%M:%SZ))"
      DCI_GATE=v3 DCI_FORCE="$FORCE" \

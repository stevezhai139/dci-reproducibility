...
- **Offline pipeline** (everything except §6.10-live): Python ≥ 3.10 with
...
- **Live end-to-end** (§6.10): additionally `pip install -r
...
previews or smoke tests and are not paper inputs. Live runs (§6.10) are
...
| v1 live, ordinary schedule (archive; superseded in the paper by the v3 three-arm study = Table 5) | `end_to_end/postgres/pg_adaptation.py`, `end_to_end/mongo/mongo_adaptation.py` | `end_to_end/postgres/out_PG_{gated_tau1p5,always1D_tau1e9}/`, `end_to_end/mongo/out/MG_*/` |
| v1 escalation-fraction replay (archive; v1-router diagnostics) | `escalation_replay.py` | prints PG 0.1458/0.0682, MG 0.2083/0.1364 (deterministic replay of the logged runs) |
| v1 firing-level sweep (archive; superseded in the paper by the S6 policy bench = Table 3) | `dci_resolution_e2e.py --seeds 50` | `dci_resolution_e2e_results.csv` (mixed recall 0.694/0.931/0.904; gated at 45.3% of multi-D cost) |
...
| v1 live mixed drift (tau1e9 leg = the single-direction result quoted in §6.10: recall 0.60, 10.9x stale) | `part2_preflight.sh` → `RUN_PART2.sh` → `part2_analyze.py` (schedule pre-check: `part2_validate_offline.py`) | `end_to_end/postgres/out_PART2_PG_mixed_{tau0,tau1p5,tau1e9}/`, `end_to_end/mongo/out/PART2_MG_mixed_*/`, `part2_summary.csv` |
...
**Offline-only (no database; reproduces §4–§6.9 and the offline halves of §6.10):**
